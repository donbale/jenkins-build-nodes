...
MS_Usage()
{
    ...
    echo "    --xz            : Compress using xz (default if detected)"
    echo "    --pxz           : Compress using pxz"
    echo "    --gzip          : Compress using gzip"
    echo "    --pigz          : Compress using pigz"
    echo "    --bzip2         : Compress using bzip2"
    echo "    --pbzip2        : Compress using pbzip2"
...
# Default settings
if type xz 2>&1 > /dev/null; then
    COMPRESS=xz
elif type bzip2 2>&1 > /dev/null; then
    COMPRESS=bzip2
elif type gzip 2>&1 > /dev/null; then
    COMPRESS=gzip
else
    COMPRESS=Unix
fi
